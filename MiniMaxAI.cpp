...
int MiniMaxAI(char boardAr[][3], char token)
{
	...
	bestMove  = -1000; //set to very low value to ensure valid move is chose
	openSpots = false;
	...
	else
	{
		...
		for(row = 0; row < 3; row++)
		{
			for(column = 0; column < 3; column++)
			{
				if(boardAr[row][column] == ' ')
				{
					//does artificial computer move
					boardAr[row][column] = aiPlayer;
					//runs MiniMaxPlay function
					tempMove = MiniMaxPlay(boardAr, humanPlayer);

					//checks for highest possible value since this is the comp
					//artificial move
